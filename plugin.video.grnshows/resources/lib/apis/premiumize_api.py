...
class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('grnshows.pm.token', 'empty_setting')

	def auth(self):
		self.token = ''
		client_id = get_setting('grnshows.pm.client_id', 'empty_setting')
		if client_id in ('empty_setting', ''): return notification('Please set a valid Premiumize Client ID', 3000)
		line = '%s[CR]%s[CR]%s'
	...
	def headers(self):
		return {'User-Agent': 'GRN Shows for Kodi', 'Authorization': 'Bearer %s' % self.token}
