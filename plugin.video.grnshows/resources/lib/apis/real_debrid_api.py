...
class RealDebridAPI:
	def __init__(self):
		self.client_ID = get_setting('grnshows.rd.client_id', 'empty_setting')
		url = {'true': 'app.real-debrid.com', 'false': 'api.real-debrid.com'}[get_setting('grnshows.rd.alternate_base_url', 'false')]
		self.base_url = 'https://%s/rest/1.0/' % url
		self.auth_url = 'https://%s/oauth/v2/' % url
		self.token = get_setting('grnshows.rd.token', 'empty_setting')
		self.secret = get_setting('grnshows.rd.secret', 'empty_setting')
		self.refresh = get_setting('grnshows.rd.refresh', 'empty_setting')
		self.device_code = ''
	...
	def auth(self):
		self.secret = ''
		self.client_ID = get_setting('grnshows.rd.client_id', 'empty_setting')
		if self.client_ID in ('empty_setting', ''): return notification('Please set a valid Real-Debrid Client ID', 3000)
