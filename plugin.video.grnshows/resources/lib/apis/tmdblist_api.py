...
class TMDbListAPI:
	def __init__(self):
		...
		self.read_access_token = get_setting('grnshows.tmdb.read_access_token', 'empty_setting')
	
	def auth(self):
		import requests
		if self.read_access_token in ('empty_setting', ''): return notification('Please set a valid TMDb Read Access Token')
	...
	def get_user_lists(self):
		...
		def _process(dummy):
			...
			return results
		account_id = get_setting('grnshows.tmdb.account_id')
	...
	def request_data(self, url, params=None, data=None, method='get'):
		headers = {'accept': 'application/json', 'content-type': 'application/json', 'Authorization': 'Bearer %s' % get_setting('grnshows.tmdb.token')}
