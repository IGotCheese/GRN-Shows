...
def window_theme_choice(params):
	if params['type'] == 'theme':
		...
		window_theme_opacity = get_setting('grnshows.window_theme_opacity', 'CC')
		...
		window_theme = get_setting('grnshows.window_theme', 'FF1F2020')[2:]
		window_theme_contrast = get_setting('grnshows.window_theme_contrast', 'FF4a4347')
...
def language_invoker_choice(params):
	...
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.grnshows/addon.xml')
...
def addon_icon_choice(params):
	...
	if set_icon: new_name = set_icon
		...
		try:
			...
			if not all_icons: kodi_utils.ok_dialog(heading='GRN Shows Icon Images', text='Error Fetching Icon Images')
		...
		except: return kodi_utils.ok_dialog(heading='GRN Shows Icon Images', text='Error Fetching Addon Icon Images') 
...
def personallists_manager_choice(params):
	...
	list_type = params['list_type']
	all_lists = get_all_personal_lists(get_setting('grnshows.personal_list.list_sort', '0'))
	choices = []
...
def set_quality_choice(params):
	...
	fl = ['4K', '1080p', '720p', 'SD']
	q_setting = get_setting('grnshows.%s' % quality_setting).split(', ')
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('grnshows.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('grnshows.', ''), default_setting_values(setting_id)['setting_default'])
					button_action = get_setting('grnshows.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def extras_ratings_choice(params={}):
	...
	list_items = [{'line1': i[0], 'icon': 'grnshows_flags/ratings/%s' % i[2]} for i in choices]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting('grnshows.%s' % filter_setting_id).split(', ')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or kodi_utils.get_icon('grnshows')
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('grnshows.%s' % setting), default_setting_values(setting)['setting_default']
...
def options_menu_choice(params, meta=None):
	...
	single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_grnshows', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
