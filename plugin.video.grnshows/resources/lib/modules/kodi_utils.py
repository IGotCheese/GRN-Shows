...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.grnshows').getAddonInfo(info)

def addon_version():
	return get_property('grnshows.addon_version') or addon_info('version')

def addon_path():
	return get_property('grnshows.addon_path') or addon_info('path')

def addon_profile():
	return get_property('grnshows.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('grnshows.addon_icon') or translate_path(addon_info('icon'))

def addon_icon_mini():
	return get_property('grnshows.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
														os.path.basename(translate_path(addon_info('icon'))))

def addon_fanart():
	return get_property('grnshows.addon_fanart') or translate_path(addon_info('fanart'))
...
def build_url(url_params):
	return 'plugin://plugin.video.grnshows/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('grnshows.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('grnshows.%s' % view_type) or None
...
def addon(addon_id='plugin.video.grnshows'):
...
def run_addon(addon='plugin.video.grnshows', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'grnshows' not in get_infolabel('Container.PluginName')
...
def refresh_widgets():
	...
	kodi_refresh()
	if get_setting('grnshows.widget_refresh_notification', 'true') == 'true': notification('Widgets Refreshed', 2500)
...
def disable_enable_addon(addon_name='plugin.video.grnshows'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.grnshows'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('grnshows.external_scraper.module')
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('GRN Shows', line1, icon or addon_icon(), time)
...
def player_check(mode, params):
	...
	if mode == 'playback.%s' % playback_key():
		...
		GRNShowsPlayer().run(params.get('url', None), params.get('obj', None))
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('grnshows.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('grnshows.playback.volumecheck_percent') or '50'), 100)
