...
def get_versions():
	try:
		result = requests.get(get_location('grnshowsam_version'))
...
def get_changes(online_version=None):
	try:
		if not online_version:
			...
			if not version_check(current_version, online_version): return kodi_utils.ok_dialog(heading='GRN Shows Updater',
				text='You are running the current version of GRN Shows.[CR][CR]There is no new version changelog to view.')
		kodi_utils.show_busy_dialog()
		result = requests.get(get_location('grnshowsam_changes'))
...
def update_check(action=4):
	...
	if not version_check(current_version, online_version):
		if action == 4: return kodi_utils.ok_dialog(heading='GRN Shows Updater', text='Installed Version: [B]%s[/B][CR]Online Version: [B]%s[/B][CR][CR] %s' \
			% (current_version, online_version, '[B]No Update Available[/B]'))
		return
	if action in (0, 4):
		if not kodi_utils.confirm_dialog(heading='GRN Shows Updater', text='Installed Version: [B]%s[/B][CR]Online Version: [B]%s[/B][CR][CR] %s' \
			% (current_version, online_version, '[B]An Update is Available[/B][CR]Perform Update?'), ok_label='Yes', cancel_label='No'): return
		if kodi_utils.confirm_dialog(heading='GRN Shows Updater', text='Do you want to view the changelog for the new release before installing?', ok_label='Yes', cancel_label='No'):
			get_changes(online_version)
			if not kodi_utils.confirm_dialog(heading='GRN Shows Updater', text='Continue with Update After Viewing Changes?', ok_label='Yes', cancel_label='No'): return
			show_after_action = False
	if action == 1: kodi_utils.notification('GRN Shows Update Occuring', icon=kodi_utils.get_icon('downloads'))
	elif action == 2: return kodi_utils.notification('GRN Shows Update Available', icon=kodi_utils.get_icon('downloads'))
...
def rollback_check():
	...
	if not kodi_utils.confirm_dialog(heading='GRN Shows Updater',
		text='Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.[CR]GRN Shows will set your update action to [B]OFF[/B] if rollback is successful' \
...
def update_addon(new_version, action, show_after_action=True):
	...
	kodi_utils.notification('GRN Shows Performing Rollback' if action == 5 else 'GRN Shows Performing Update', icon=kodi_utils.get_icon('downloads'))
	zip_name = 'plugin.video.grnshows-%s.zip' % new_version
	url = get_location('%s') % zip_name
	kodi_utils.show_busy_dialog()
	result = requests.get(url, stream=True)
	kodi_utils.hide_busy_dialog()
	if result.status_code != 200: return kodi_utils.ok_dialog(heading='GRN Shows Updater', text='Error Updating.[CR]Please install new update manually')
	...
	shutil.rmtree(path.join(kodi_utils.translate_path('special://home/addons/'), 'plugin.video.grnshows'))
	success = unzip(zip_location, kodi_utils.translate_path('special://home/addons/'), kodi_utils.translate_path('special://home/addons/plugin.video.grnshows/'))
	kodi_utils.delete_file(zip_location)
	if not success: return kodi_utils.ok_dialog(heading='GRN Shows Updater', text='Error Updating.[CR]Please install new update manually')
	if action == 5:
		set_setting('update.action', '3')
		kodi_utils.ok_dialog(heading='GRN Shows Updater', text='[CR]Success.[CR]GRN Shows rolled back to version [B]%s[/B]' % new_version)
	elif action in (0, 4):
		if show_after_action:
			if kodi_utils.confirm_dialog(heading='GRN Shows Updater', text='[CR]Success.[CR]GRN Shows updated to version [B]%s[/B]' % new_version,
										ok_label='Changelog', cancel_label='Exit', default_control=10) != False:
				kodi_utils.show_text('Changelog', file=kodi_utils.translate_path('special://home/addons/plugin.video.grnshows/resources/text/changelog.txt'), font_size='large')
		else:
			kodi_utils.ok_dialog(heading='GRN Shows Updater', text='[CR]Success.[CR]GRN Shows updated to version [B]%s[/B]' % new_version)
